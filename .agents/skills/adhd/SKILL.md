...
1. **Score.** Rate each idea on three axes 0 to 10: novelty (distance from
 ...
 address the stated problem). Scores are judgments, not measurements. For
 any idea that looks attractive but is a trap (hidden cost, false economy,
 will not scale, premature abstraction), flag it with a one-line reason.
...
3. **Deepen the top 3.** Rank by weighted score (novelty 0.35 + viability
 0.40 + fit 0.25), exclude traps, take top 3. The obvious conventional
 answer stays eligible: it was banned during generation, not in the final
 comparison. Ensure it appears in the report with its honest score, so the
 user can pick the boring reliable option over a novel one.
 For each of the top 3, spawn one Agent call that produces:
