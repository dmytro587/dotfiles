---
name: review
description: Review the changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the code follow this repo's documented coding standards?) and Spec (does the code match what the originating issue/PRD asked for?). Runs both reviews in parallel sub-agents and reports them side by side. Reviews committed changes only by default; uncommitted work needs an explicit worktree mode.
---
...
If `docs/agents/issue-tracker.md` is missing, ask the user where issues live; do not run any setup command during a read-only review.
...
Capture the diff command once: `git diff <fixed-point>...HEAD` (three-dot, so the comparison is against the merge-base). Also note the list of commits via `git log <fixed-point>..HEAD --oneline`. This reviews committed changes only. If the user asked to review work-in-progress (uncommitted) changes, use worktree mode instead: review `git diff` (unstaged), `git diff --staged` (staged), and ask whether untracked files belong. State explicitly which mode is in effect and what it excludes.

### 2. Identify the spec source

Look for the originating spec, in this order:

1. A path the user passed as an argument (authoritative when given).
...
4. If nothing is found or candidates conflict, ask the user which spec is authoritative. If they say there isn't one, the **Spec** sub-agent will skip and report "no spec available".
